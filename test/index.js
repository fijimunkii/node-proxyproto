const proxyProtocolParser = require('../index');
...
module.exports = async t => {
  ...
  t.test('returns a net.Server instance', async (t) => {
    t.type(proxyProtocolParser(httpServer), 'Server');
  });

  t.test('server interface must be supplied', async (t) => {
    t.throws(() => proxyProtocolParser());
    t.doesNotThrow(() => proxyProtocolParser(httpServer));
  });
  ...
  t.test('vanilla http connections are untouched', async (t) => {
    await Promise.all([
      ...
      new Promise(resolve => {
        ...
        const proxied = proxyProtocolParser(server);
        proxied.listen(PORT);
      }),
  ...
  t.test('http - PROXY protocol headers are parsed', async (t) => {
    await new Promise(resolve => {
      ...
      const proxied = injectProxyHeaders(proxyProtocolParser(server));
      proxied.listen(PORT);
